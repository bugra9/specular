...
export function setUsb(v) {
  return (dispatch, getState) => {
    ...
    if(v != " ") {
      ...
      const child = execFile('/opt/specular/cli/specular', ["--partitions", device]);
...
export function getDevices() {
  return (dispatch, getState) => {
    const child = execFile('/opt/specular/cli/specular', ['--silent', '--listDevices']);
    child.stdout.on('data', (data) => {
      console.log(data);
      let arr = data.split('\n');
      arr.pop();
...
export function burn(v) {
  return (dispatch, getState) => {
    ...
    if(system.mode == "direct") {
      ...
      exec(["rsync", "-rlt", "--info=progress2", iso, system.usbDataPath+"/specular/linux/"])(dispatch, getState);
    }
    else {
      let config = {};
       switch(system.mode) {
      ...
      if(iso == "" || iso == undefined)
        exec(['/opt/specular/cli/specular', '--silent', '--name', system.name, '--type', config.type, '--scheme', config.scheme, '--install', getState().system.usbPath], null, true)(dispatch, getState);
      else
        exec(['/opt/specular/cli/specular', '--silent', '--name', system.name, '--type', config.type, '--scheme', config.scheme, '--full', iso, getState().system.usbPath], null, true)(dispatch, getState);
